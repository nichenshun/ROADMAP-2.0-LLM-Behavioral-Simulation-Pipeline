# #!/usr/bin/env python3
# """
# Build participants_demographics.json from raw CSVs under ./data.
...
# # --------------------
# # Config: input CSVs in ./data and output JSON
# # --------------------
# DATA_DIR = Path("data")
...
#                 pid = str(row["STUDY_PRTCPT_ID"])
#                 day = row["DaysFromTransplant"]
#                 steps = safe_float(row["total_steps"])

#                 if pd.isna(day) or steps is None:
#                     continue
#                 try:
#                     day_int = int(day)
#                 except Exception:
...
#                 if pd.isna(day) or mood_val is None:
#                     continue
#                 try:
#                     day_int = int(day)
#                 except Exception:
...
#     print(f"[OK] Wrote participants_demographics.json to {out_path.resolve()}")
